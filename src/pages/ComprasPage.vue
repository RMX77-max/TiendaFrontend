<template>
  <q-page class="pagina-compras q-pa-lg">
    <div>
      ...
      <h1 class="text-h4 text-weight-bold q-mt-sm q-mb-sm">Registro de compras y pedidos</h1>
      <p class="text-body1 text-grey-8 q-ma-none">
        Vista simple para registrar compras, productos y abonos por sucursal.
      </p>
    </div>
      ...
      <q-card flat bordered class="tarjeta-compra-principal">
          ...
          <div class="cabecera-tarjeta-detalle cabecera-tarjeta-detalle--envolvente">
            <div>
              <div class="text-h6 text-weight-bold">
                {{ compraEditandoId ? `Editar compra #${compraEditandoId}` : 'Nueva compra / pedido' }}
              </div>
              <p class="text-body2 text-grey-7 q-mt-sm q-mb-none">
                La compra se registra por cabecera, detalle y abonos cuando corresponda.
              </p>
            </div>
        ...
        <q-card-section class="q-pa-lg">
          <div class="rejilla-compras-simple">
            ...
            >
              <template #after>
                <q-btn flat round dense icon="add" @click="abrirDialogoProveedor" />
              </template>
            </q-select>
          ...
          <div class="cabecera-tarjeta-detalle cabecera-tarjeta-detalle--envolvente">
            ...
            <div class="row q-gutter-sm items-center">
              <q-chip square color="grey-2" text-color="dark">
                Total: $ {{ formatearMonto(totalGeneralUsd) }} / Bs. {{ formatearMonto(totalGeneralBs) }}
              </q-chip>
              <q-btn flat color="primary" icon="add" label="Agregar fila" @click="agregarFilaDetalle" />
            </div>
          </div>
            ...
            <q-markup-table flat bordered class="tabla-simple-compras">
              <thead>
                <tr>
                  ...
                  <th class="text-left">Detalle libre</th>
              ...
              <tbody>
                <tr v-for="(detalle, indice) in formulario.detalles" :key="detalle.uid">
                  <td class="columna-tabla-simple">
                    <q-input v-model.number="detalle.cantidad" dense outlined type="number" min="1" @update:model-value="recalcularFila(detalle)" />
                  </td>
                  <td class="columna-tabla-simple columna-tabla-simple--producto">
                    <q-select
                      ...
                      emit-value
                      map-options
                      :options="opcionesProductosFiltradas[indice] || productos"
                      option-value="value"
                      option-label="label"
                      @filter="(valor, actualizar) => filtrarProductos(indice, valor, actualizar)"
                      @update:model-value="seleccionarProductoDetalle(detalle, $event)"
                    />
                  </td>
                  <td class="columna-tabla-simple columna-tabla-simple--detalle">
                    <q-input v-model="detalle.detalleTexto" dense outlined />
                  </td>
                  <td class="columna-tabla-simple">
                    <q-input v-model.number="detalle.tipoCambioAplicado" dense outlined type="number" min="0.01" step="0.0001" @update:model-value="recalcularFila(detalle)" />
                  </td>
                  <td class="columna-tabla-simple">
                    <q-input :model-value="detalle.precioUnitarioUsd" dense outlined type="number" min="0" step="0.0001" @update:model-value="actualizarMontoDetalle(detalle, 'usd', $event)" />
                  </td>
                  <td class="columna-tabla-simple">
                    <q-input :model-value="detalle.precioUnitarioBs" dense outlined type="number" min="0" step="0.0001" @update:model-value="actualizarMontoDetalle(detalle, 'bs', $event)" />
                  </td>
                  <td class="columna-tabla-simple">
                    <q-input :model-value="formatearMonto(detalle.subtotalUsd)" dense outlined disable />
                  </td>
                  <td class="columna-tabla-simple">
                    <q-input :model-value="formatearMonto(detalle.subtotalBs)" dense outlined disable />
                  </td>
                  <td class="columna-tabla-simple columna-tabla-simple--detalle">
                    <q-input v-model="detalle.observaciones" dense outlined />
                  </td>
                  <td class="text-center columna-tabla-simple">
                    <q-btn flat round dense icon="delete" color="negative" :disable="formulario.detalles.length === 1" @click="eliminarFilaDetalle(indice)" />
                  </td>
                </tr>
              </tbody>
            </q-markup-table>
        ...
        <q-card-section v-if="mostrarAbonosDirectos" class="q-pa-lg">
          <div class="cabecera-tarjeta-detalle cabecera-tarjeta-detalle--envolvente">
            ...
            <div class="row q-gutter-sm items-center">
              <q-chip square color="grey-2" text-color="dark">
                Abonos: $ {{ formatearMonto(totalAbonosUsd) }} / Bs. {{ formatearMonto(totalAbonosBs) }}
              </q-chip>
              <q-chip square color="grey-2" text-color="dark">
                Saldo: $ {{ formatearMonto(saldoPendienteUsd) }} / Bs. {{ formatearMonto(saldoPendienteBs) }}
              </q-chip>
              <q-chip
                square
                :color="diferenciaCambiariaBs > 0 ? 'red-1' : diferenciaCambiariaBs < 0 ? 'green-1' : 'grey-2'"
                :text-color="diferenciaCambiariaBs > 0 ? 'red-10' : diferenciaCambiariaBs < 0 ? 'green-10' : 'dark'"
              >
          ...
          <div class="contenedor-tabla-simple q-mt-lg">
            <q-markup-table flat bordered class="tabla-simple-compras">
              ...
              <tbody>
                <tr v-for="(abono, indice) in formulario.abonos" :key="abono.uid">
                  <td class="columna-tabla-simple columna-tabla-simple--producto">
                    <q-select v-model="abono.sucursalId" dense outlined emit-value map-options :options="sucursales" option-value="value" option-label="label" />
                  </td>
                  <td class="columna-tabla-simple">
                    <q-input v-model="abono.fechaAbono" dense outlined type="date" />
                  </td>
                  <td class="columna-tabla-simple">
                    <q-input v-model.number="abono.tipoCambioAbono" dense outlined type="number" min="0.01" step="0.0001" @update:model-value="recalcularAbono(abono)" />
                  </td>
                  <td class="columna-tabla-simple">
                    <q-input :model-value="abono.abonoUsd" dense outlined type="number" min="0" step="0.0001" @update:model-value="actualizarMontoAbono(abono, 'usd', $event)" />
                  </td>
                  <td class="columna-tabla-simple">
                  ...
                  <td class="text-center columna-tabla-simple">
                    <q-btn flat round dense icon="delete" color="negative" :disable="formulario.abonos.length === 1" @click="eliminarFilaAbono(indice)" />
                  </td>
                </tr>
              </tbody>
            </q-markup-table>
    ...
    <q-dialog v-model="dialogoProveedorAbierto">
      <q-card class="tarjeta-dialogo-transferencia">
        ...
        <q-card-section class="q-pa-lg">
          <q-banner v-if="errorProveedor" rounded class="bg-red-1 text-red-10 q-mb-lg">
            {{ errorProveedor }}
          </q-banner>
        ...
        <q-card-actions align="right" class="q-pa-lg">
          <q-btn flat color="grey-8" label="Cancelar" @click="cerrarDialogoProveedor" />
          <q-btn
            unelevated
            color="dark"
            text-color="white"
...
const proveedoresFiltrados = computed(() => {
  ...
  return proveedores.value.filter((proveedor) =>
    String(proveedor.label || '').toLowerCase().includes(termino)
  )
})

const totalGeneralUsd = computed(() =>
  formulario.detalles.reduce((acumulado, detalle) => acumulado + Number(detalle.subtotalUsd || 0), 0)
)

const totalGeneralBs = computed(() =>
  formulario.detalles.reduce((acumulado, detalle) => acumulado + Number(detalle.subtotalBs || 0), 0)
)

const totalAbonosUsd = computed(() =>
  formulario.abonos.reduce((acumulado, abono) => acumulado + Number(abono.abonoUsd || 0), 0)
)

const totalAbonosBs = computed(() =>
  formulario.abonos.reduce((acumulado, abono) => acumulado + Number(abono.abonoBs || 0), 0)
)
...
const saldoPendienteUsd = computed(() => Math.max(0, redondearMonto(totalGeneralUsd.value - totalAbonosUsd.value)))
...
const diferenciaCambiariaBs = computed(() => {
  ...
  return redondearMonto(totalAbonosBs.value - costoReferencialBs)
})
const etiquetaDiferenciaCambiaria = computed(() => {
  if (diferenciaCambiariaBs.value > 0) {
    return `Perdida cambiaria: Bs. ${formatearMonto(diferenciaCambiariaBs.value)}`
  }

  if (diferenciaCambiariaBs.value < 0) {
    return `Ahorro cambiario: Bs. ${formatearMonto(Math.abs(diferenciaCambiariaBs.value))}`
  }
...
function recalcularFila (detalle) {
  ...
  const tipoCambio = Math.max(0.0001, normalizarNumero(detalle.tipoCambioAplicado))
  ...
  if (detalle.monedaReferencia === 'bs') {
    ...
    detalle.precioUnitarioUsd = redondearMonto(detalle.precioUnitarioBs / tipoCambio)
  } else {
    detalle.monedaReferencia = 'usd'
    detalle.precioUnitarioUsd = redondearMonto(detalle.precioUnitarioUsd)
    detalle.precioUnitarioBs = redondearMonto(detalle.precioUnitarioUsd * tipoCambio)
  }
...
function filtrarProductos (indice, valor, actualizar) {
  actualizar(() => {
    ...
    opcionesProductosFiltradas[indice] = productos.value.filter((producto) => {
      const texto = `${producto.modelo || ''} ${producto.marca || ''} ${producto.categoria || ''}`.toLowerCase()
...
function seleccionarProductoDetalle (detalle, idProducto) {
  const producto = productos.value.find((item) => Number(item.id) === Number(idProducto))
  ...
  detalle.precioUnitarioUsd = Number(producto.costo_usd || producto.precio_unitario_usd || 0)
  detalle.monedaReferencia = 'usd'
  recalcularFila(detalle)
}
...
function construirPayloadCompra () {
  return {
    ...
    tipo_cambio_general: 9,
    tiempo_entrega_dias: Math.max(0, Math.trunc(Number(formulario.tiempoEntregaDias || 0))),
...
function validarFormularioAntesDeGuardar () {
  ...
  const filaInvalida = formulario.detalles.find((detalle) =>
    !detalle.detalleTexto.trim() && !detalle.productoId
  )
  ...
  if (mostrarAbonosDirectos.value) {
    const abonoSinSucursal = formulario.abonos.find((abono) => !abono.sucursalId)
...
async function guardarCompra () {
  ...
  try {
    ...
    mostrarNotificacion('positive', respuesta.message || 'Compra guardada correctamente.')
  } catch (error) {
...
async function guardarProveedor () {
  ...
  if (!formularioProveedor.nombre.trim()) {
    errorProveedor.value = 'Ingresa el nombre del proveedor o de la persona de compra.'
    return
  }
  ...
  try {
    ...
    mostrarNotificacion('positive', respuesta.message || 'Proveedor registrado correctamente.')
  } catch (error) {
    errorProveedor.value = error.message || 'No se pudo registrar el proveedor.'
  } finally {
